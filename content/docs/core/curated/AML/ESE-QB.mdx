...
1. **Random Component** — The distribution of the response variable $Y$, which must belong to the **exponential family** (e.g., Normal, Bernoulli, Poisson).

2. **Systematic Component** — A linear combination of input variables called the **linear predictor**: $\eta = \beta_0 + \beta_1 x_1 + \cdots + \beta_p x_p$

3. **Link Function** — Connects the mean $\mu = E(Y)$ to the linear predictor via $g(\mu) = \eta$. It keeps predictions within a valid range.
...
**How They Interact:** The systematic component builds $\eta$ from the predictors. The link function maps $\eta$ to the mean $\mu$. The random component links $\mu$ to the observed $Y$. This lets one framework handle continuous, binary, and count data.
...
**Probit regression** is a GLM for binary classification. It models probability using the **standard normal CDF (bell curve area)**. The link function is the inverse CDF: $g(\mu) = \Phi^{-1}(\mu)$.

**Logistic regression** also handles binary classification but uses the **sigmoid function** and the logit link: $g(\mu) = \log\frac{\mu}{1-\mu}$.
...
<LongQuestion>
### Q7. (Extras for Reference/Practice) For the given relations: Socioeconomic Status (S) affects Education Level (E) and Nutrition (N); Education Level (E) affects Job Type (J); Nutrition (N) affects Health Condition (H); Job Type (J) and Health Condition (H) affect Income (I). Draw the DAG, write the joint probability, and write three conditional independence relations. <QuestionDot color="yellow" text="IA1-QB" /> <QuestionDot color="cyan" text="MSE-QB" /> <QuestionDot color="purple" text="Asked in MSE" />
</LongQuestion>
...
Given $N$ i.i.d. samples from $\mathcal{N}(\mu, \sigma^2)$, we maximize the log-likelihood.
...
**MLE for $\mu$** — set $\frac{\partial \ell}{\partial \mu} = 0$:
$$
\boxed{\hat{\mu}_{\text{MLE}} = \frac{1}{N}\sum_{i=1}^{N} y_i} \quad \text{(sample mean)}
$$

**MLE for $\sigma^2$** — set $\frac{\partial \ell}{\partial \sigma^2} = 0$:
$$
\boxed{\hat{\sigma}^2_{\text{MLE}} = \frac{1}{N}\sum_{i=1}^{N}(y_i - \hat{\mu})^2} \quad \text{(biased sample variance — uses } N \text{, not } N-1)
$$
...
Compute the **responsibility** $\gamma_{nk}$ — the probability that data point $x_n$ belongs to component $k$:

$$
\gamma_{nk} = \frac{\pi_k \, \mathcal{N}(x_n \mid \mu_k, \Sigma_k)}{\sum_{j=1}^{K} \pi_j \, \mathcal{N}(x_n \mid \mu_j, \Sigma_j)}
$$

This assigns **soft (fractional) membership** to each data point instead of hard labels.

**M-Step (Maximization):**

Using the responsibilities, update all three parameters:
- $N_k = \sum_n \gamma_{nk}$ (effective count for cluster $k$)
- $\hat{\mu}_k = \frac{1}{N_k}\sum_n \gamma_{nk} x_n$ (weighted mean)
- $\hat{\Sigma}_k = \frac{1}{N_k}\sum_n \gamma_{nk}(x_n - \hat{\mu}_k)(x_n - \hat{\mu}_k)^T$ (weighted covariance)
- $\hat{\pi}_k = \frac{N_k}{N}$ (updated mixing weight)

**Why These Steps Are Crucial:**

- With **known** cluster labels, MLE is easy (just compute sample statistics per cluster).
- With **hidden** labels, the log-likelihood is **non-convex** — direct optimization fails.
- EM alternates between estimating the missing information (E) and maximizing with it (M).
...
*(See Q3 above for the core E-step and M-step formulas.)*

**E-Step:** Compute responsibility $\gamma_{nk}$ = probability that $x_n$ belongs to component $k$, using current parameters. This gives a **soft assignment** for each data point.

**M-Step:** Update parameters using the responsibilities as weights:
- $\hat{\mu}_k$ = responsibility-weighted mean
- $\hat{\pi}_k = N_k / N$ = updated mixing proportion

**Why EM Converges to a Local Optimum:**

1. **Monotonic increase:** Every iteration increases (or keeps equal) the log-likelihood — it never decreases.
2. **Bounded above:** Log-likelihood is bounded (probabilities $\le 1$), so it must converge.
3. **Jensen's Inequality:** The E-step builds a lower bound on the log-likelihood; the M-step maximizes that bound.

EM finds a **local optimum** (not necessarily global) because the likelihood surface is non-convex. Starting from different initial parameters may give different results.
...
A **Conditional Random Field (CRF)** is an undirected probabilistic graphical model that directly models $P(Y \mid X)$ — the conditional distribution of output labels $Y$ given observed inputs $X$.
...
where $\psi_C$ are potential functions depending on labels and the entire input $X$, and $Z(X)$ is the normalization constant.

**Why CRFs Are Used in Sequence Modeling:**

1. **Discriminative:** Models $P(Y \mid X)$ directly — no need to model $P(X)$.
2. **Rich features:** Can use arbitrary, overlapping properties of the full input sequence (word shape, context, prefix, etc.) — more expressive than HMMs.
3. **No label bias:** CRFs avoid the label bias problem that affects HMMs.
4. **Efficient inference:** Viterbi (MAP) and Forward-Backward (marginals) work in $O(N^2 T)$.
5. **Applications:** POS tagging, Named Entity Recognition (NER), chunking, image segmentation.
...
**Rejection Sampling:**
- **Goal:** Produce actual i.i.d. samples from $p(x)$.
- **Mechanism:** Accept/reject proposals based on $\frac{p(x^*)}{M \cdot q(x^*)}$.
- **Output:** True samples from $p(x)$ — usable for any downstream purpose.
- **Requires:** Envelope $p(x) \le M \cdot q(x)$ for all $x$.
- **Example:** Sampling from a Gaussian $p(x) \propto e^{-x^2/2}$ using a Uniform$(-10, 10)$ proposal.

**Importance Sampling:**
- **Goal:** Estimate **expectations** $\mathbb{E}_p[f(x)]$ without generating true samples from $p(x)$.
- **Mechanism:** Draw samples from $q(x)$ and weight them: $w(x) = \frac{p(x)}{q(x)}$:
$$
\hat{\mu}_{\text{IS}} = \frac{\sum_{i} f(x_i) w(x_i)}{\sum_{i} w(x_i)}, \quad x_i \sim q(x)
$$
- **Output:** Weighted estimate — not true samples, but sufficient for expectations.
- **Requires:** $q(x) > 0$ wherever $p(x) > 0$; no envelope constant needed.
- **Example:** Estimating $\mathbb{E}_p[x^2]$ with $p = \mathcal{N}(0,1)$ using a wider $q = \mathcal{N}(0,4)$.
...
| Output | i.i.d. samples | Weighted samples |
...
| When to use | Low-dim, close proposal | Expectation estimation, reusing samples |
